...
class Card {
	...
	valueCompare(other) {
		return IntCompare(this.numericValue, other.numericValue);
	}
}
...
class HandType {
	...
	compare(other) {
		return IntCompare(this.level, other.level);
	}
}
...
class Hand {
	analyzeHand() {
		...
		let card3 = this.card3;
		let card4 = this.card4;
		let card5 = this.card5;
	...
	constructor(card1, card2, card3, card4, card5) {
		...
		this.card4 = card4;
		this.card5 = card5;
		this.handType = this.analyzeHand();
	}

	compare(other) {
		function defaultCompareEqualHandTypes(self) {
			let five = self.card5.valueCompare(other.card5);
			if (five !== Order.Equal) {
				return five;
			}
			let four = self.card4.valueCompare(other.card4);
			if (four !== Order.Equal) {
				return four;
			}
			let three = self.card3.valueCompare(other.card3);
			if (three !== Order.Equal) {
				return three;
			}
			let two = self.card2.valueCompare(other.card2);
			if (two !== Order.Equal) {
				return two;
			}
			return self.card1.valueCompare(other.card1);
		}
		function compareStraight(self) {
			...
			return self.card5.valueCompare(other.card5);
		}
		function compareFourOfAKind(self) {
			...
			let two = duplicatedUs.valueCompare(duplicatedThem);
			if (two !== Order.Equal) {
				return two;
			}
			return findNonDuplicated(self.card1, self.card3, self.card5).valueCompare(findNonDuplicated(other.card1, other.card3, other.card5));
		}
		function compareFullHouse(self) {
			...
			let two = usThree.valueCompare(themThree);
			if (two !== Order.Equal) {
				return two;
			}
			return findTwo(self.card1, self.card3, self.card5).valueCompare(findTwo(other.card1, other.card3, other.card5));
		}
		function compareThreeOfAKind(self) {
			...
			let three = usThree.valueCompare(themThree);
			if (three !== Order.Equal) {
				return three;
			}
			...
			let two = usKicker1.valueCompare(themKicker1);
			if (two !== Order.Equal) {
				return two;
			}
			return findSecondKicker(self.card1, self.card2, self.card4).valueCompare(findSecondKicker(other.card1, other.card2, other.card4));
		}
		function compareTwoPair(self) {
			...
			let three = ourFirstPair.valueCompare(theirFirstPair);
			if (three !== Order.Equal) {
				return three;
			}
			let ourNextPair = findLowerPair(self);
			let theirNextPair = findLowerPair(other);
			let two = ourNextPair.valueCompare(theirNextPair);
			if (two !== Order.Equal) {
				return two;
			}
			...
			return ourKicker.valueCompare(theirKicker);
		}
		...
		function comparePairs(self) {
			...
			let pair = ourPair.valueCompare(theirPair);
			if (pair !== Order.Equal) {
				return pair;
			}
			...
			let kicker1 = ourKicker1.valueCompare(theirKicker1);
			...
			let kicker2 = ourKicker2.valueCompare(theirKicker2);
			...
			return ourKicker3.valueCompare(theirKicker3);
		}

		switch (this.handType.compare(other.handType)) {
			case Order.Less: 
				return Order.Less;
			case Order.Greater:
				return Order.Greater;
			case Order.Equal:
				switch (this.handType) {
					case HandType.Pair:
						return comparePairs(this);
					case HandType.TwoPair:
						return compareTwoPair(this);
					case HandType.ThreeOfAKind:
						return compareThreeOfAKind(this);
					case HandType.FullHouse:
						return compareFullHouse(this);
					case HandType.FourOfAKind:
						return compareFourOfAKind(this);
					case HandType.Straight:
						return compareStraight(this);
					case HandType.StraightFlush:
						return compareStraight(this);
					default:
						return defaultCompareEqualHandTypes(this);
				}
		}
	}
}
...
class UnknownPocketHand {
	...
	getAllCorrespondingRealHands() {
		...
		let val1 = this.name[0];
		let val2 = this.name[1];
		if (val1 === val2) {
...
class Range {
	...
	iterateRange() {
		var pocketHands = [];
		for (var i = 0; i < this.unknownPocketHands.length; i++) {
			let unknownHand = this.unknownPocketHands[i];
			pocketHands = pocketHands.concat(unknownHand.getAllCorrespondingRealHands());
		}
...
class Spot {
	...
	notLoseWith(opcard1, opcard2, table1, table2, table3, table4, table5) {
		let opponentsHandsFrom = [opcard1, opcard2, table1, table2, table3, table4, table5];
		let myHandsFrom = [this.myPocketHand.card1, this.myPocketHand.card2, table1, table2, table3, table4, table5];
		...
		switch (myBestHand.compare(opponentBestHand)) {
			case Order.Greater:
				return true;
			case Order.Equal:
				return true;
	...
	getTablePossibilities(opponentPocketHand) {
		let deck = getDeck();
		deck = deck.filter(card => card !== this.myPocketHand.card1 && card !== this.myPocketHand.card2 && card !== opponentPocketHand.card1 && card !== opponentPocketHand.card2);	
		if (this.flop === undefined) {
			return iterateSubListsWithSize(deck, 5);
		}
		else if (this.turn === undefined) {
			deck = deck.filter(card => card !== this.flop.card1 && card !== this.flop.card2 && card !== this.flop.card3);
			return iterateSubListsWithSize(deck, 2).map(L => L.concat([this.flop.card1, this.flop.card2, this.flop.card3]));
		}
		else {
			let table = [this.flop.card1, this.flop.card2, this.flop.card3, this.turn];
			deck = deck.filter(card => card !== this.flop.card1 && card !== this.flop.card2 && card !== this.flop.card3 && card !== turn);
			return deck.map(c => table.concat([c]));
		}
	}

	updateTablePossibilities(prev, newAdvCard1, newAdvCard2, oldAdvCard1, oldAdvCard2) {
		function updatePossibility(T) {
			//if new card was in the deck before, but now is in opponents hand,
			//replace it with the card from the opponents hand as long as 
			//the replacement is not still in the opponents hand
			if (newAdvCard1 !== oldAdvCard1 && newAdvCard1 !== oldAdvCard2 &&
				oldAdvCard1 !== newAdvCard2) {
				let index = T.indexOf(newAdvCard1);
				if (index != -1) {
					T[index] = oldAdvCard1;
				}
			}
			if (newAdvCard2 !== oldAdvCard2 && newAdvCard2 !== oldAdvCard1 &&
				oldAdvCard2 !== newAdvCard1) {
				let index = T.indexOf(newAdvCard2);
				if (index != -1) {
					T[index] = oldAdvCard2;
				}
			}
		...
		return prev.map(updatePossibility);
	}

	isOnBoard(card) {
		if (this.flop !== undefined) {
			if (this.flop.card1 === card) {
				return true;
			}
			if (this.flop.card2 === card) {
				return true;
			} 
			if (this.flop.card3 === card) {
				return true;
			}
			if (this.turn === card) {
				return true;
	...
	notInMyHand(card) {
		return card !== this.myPocketHand.card1 && card !== this.myPocketHand.card2;
	}

	getEquity() {
		var prob = 0;
		var opponentPossibilities = this.opponentRange.iterateRange();
		opponentPossibilities = opponentPossibilities.filter(L => !this.isOnBoard(L[0]) && !this.isOnBoard(L[1]) && this.notInMyHand(L[0]) && this.notInMyHand(L[1]));
		var tablePossibilities;
		var totalNumHands = 0;
		for (var j = 0; j < opponentPossibilities.length; j++) {
			let [card1, card2] = opponentPossibilities[j];
			tablePossibilities = this.getTablePossibilities(new PocketHand(card1, card2));
			...
			for (var i = 0; i < tablePossibilities.length; i++) {
				...
				if (this.notLoseWith(card1, card2, table1, table2, table3, table4, table5)) {
					prob += 1;
...
function unitTest() {
	...
	function assertlt(h1, h2) {
		assert(h1.compare(h2) === Order.Less);
		assert(h2.compare(h1) === Order.Greater);
	}
	function asserteq(h1, h2) {
		assert(h1.compare(h2) === Order.Equal);
		assert(h2.compare(h1) === Order.Equal);
	}
	...
	console.log(spot.getEquity().toString()); //should be around .4625
